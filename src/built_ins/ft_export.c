...
/*   Updated: 2026/03/14 21:48:49 by ekarout          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_export_key(t_env **exp, t_env **env, char *key)
{
	...
	if (find_key(exp, key))
		return ;
	change_exp_value(exp, env, key, NULL);
}
...
void	ft_export_key_value(t_env **exp, t_env **env, char *arg)
{
	...
	if (!is_valid_key(key_value[0]))
		...
		change_exp_value(exp, env, key_value[0], key_value[1]);
	i = -1;
	while (key_value[++i])
		free(key_value[i]);
	free(key_value);
