...
/*   Updated: 2026/04/07 10:05:22 by ekarout          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_expansion_len(char *value, int *i, t_vars vars)
{
	if (value[*i] == '$')
		return (get_value_len(value, i, vars));
	else if (value[*i] == '~')
		return (get_value_len("HOME", i, vars));
	return (0);
}
...
int	get_new_size(char *value, t_vars vars)
{
	...
	while (value[i])
	{
		...
		{
			len += get_expansion_len(value, &i, vars);
			i++;
		}
		else
		{
